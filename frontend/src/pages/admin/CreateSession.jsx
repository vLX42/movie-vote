...
import { adminPost } from '../../hooks/useApi';
import './Admin.css';
  ...
  async function handleSubmit(e) {
    ...
    try {
      const body = {
        name: form.name,
        slug: form.slug,
        votesPerVoter: parseInt(form.votesPerVoter),
        rootInviteCodes: parseInt(form.rootInviteCodes),
        guestInviteSlots: parseInt(form.guestInviteSlots),
        maxInviteDepth: form.maxInviteDepth ? parseInt(form.maxInviteDepth) : null,
        allowJellyseerrRequests: form.allowJellyseerrRequests,
        expiresAt: form.expiresAt || null
      };

      const data = await adminPost('/api/admin/sessions', body, secret);
      setResult(data);
    } catch (err) {
  ...
  if (!secret) {
    return (
      <div className="admin-page page-centered">
        <p className="label-mono text-danger">Not authenticated. <Link to="/admin">Go to Admin</Link></p>
      </div>
    );
  }
          ...
          <div className="panel">
            ...
            <div className="mt-3">
              ...
              <div className="invite-list">
                {result.rootInviteLinks.map(link => (
            ...
            <div className="flex gap-2 mt-3" style={{ flexWrap: 'wrap' }}>
              <Link to={`/admin/sessions/${result.session.id}`} className="btn btn-primary">
                Manage Session
              </Link>
    ...
    <div className="admin-page">
      ...
      <div className="admin-content">
        <form className="panel create-session-form" onSubmit={handleSubmit}>
          ...
          {error && (
            <div className="create-session-form__error label-mono text-danger mb-2">
              {error}
            </div>
          )}
