...
import './VoteTokens.css';

export default function VoteTokens({ total, used }) {
  const remaining = total - used;
  const tokens = Array.from({ length: total }, (_, i) => i < remaining);

  // Group into sets of 5 for tally display
  const groups = [];
...
function TallyGroup({ tokens, groupIndex }) {
  return (
...
function TallyMark({ active, isSlash, index }) {
  return (
    <motion.div
